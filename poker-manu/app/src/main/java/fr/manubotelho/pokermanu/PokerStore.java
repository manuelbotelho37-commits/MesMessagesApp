...
final class PokerStore extends SQLiteOpenHelper {
    static final class Session {
        ...
        Session(long id,long startedAt,long endedAt,String type,String location,String game,String stakes,double buyIn,double rebuys,double cashOut,int handsPlayed){
            ...
            this.buyIn=buyIn; this.rebuys=rebuys; this.cashOut=cashOut; this.handsPlayed=handsPlayed;
        }
    ...
    PokerStore(Context context){ super(context,"poker_manu.db",null,3); }

    @Override public void onCreate(SQLiteDatabase db){
        db.execSQL("CREATE TABLE sessions (id INTEGER PRIMARY KEY AUTOINCREMENT, started_at INTEGER NOT NULL, ended_at INTEGER NOT NULL, type TEXT NOT NULL DEFAULT 'Cash Game', location TEXT NOT NULL, game TEXT NOT NULL, stakes TEXT NOT NULL, buy_in REAL NOT NULL, rebuys REAL NOT NULL, cash_out REAL NOT NULL, hands_played INTEGER NOT NULL DEFAULT 0)");
    ...
    @Override public void onUpgrade(SQLiteDatabase db,int oldVersion,int newVersion){
        ...
        if(oldVersion<3){
            try{ db.execSQL("ALTER TABLE sessions ADD COLUMN hands_played INTEGER NOT NULL DEFAULT 0"); }catch(Exception ignored){}
        }
    }

    long saveSession(long startedAt,long endedAt,String type,String location,String game,String stakes,double buyIn,double rebuys,double cashOut,int handsPlayed){
        ...
        v.put("buy_in",Math.max(0,buyIn)); v.put("rebuys",Math.max(0,rebuys)); v.put("cash_out",Math.max(0,cashOut)); v.put("hands_played",Math.max(0,handsPlayed));
        return getWritableDatabase().insertOrThrow("sessions",null,v);
    }

    List<Session> sessions(){
        ArrayList<Session> out=new ArrayList<>();
        try(Cursor c=getReadableDatabase().query("sessions",new String[]{"id","started_at","ended_at","type","location","game","stakes","buy_in","rebuys","cash_out","hands_played"},null,null,null,null,"started_at DESC")){
            while(c.moveToNext()) out.add(new Session(c.getLong(0),c.getLong(1),c.getLong(2),c.getString(3),c.getString(4),c.getString(5),c.getString(6),c.getDouble(7),c.getDouble(8),c.getDouble(9),c.getInt(10)));
        }
        return out;
    }

    Session lastSession(){
        try(Cursor c=getReadableDatabase().query("sessions",new String[]{"id","started_at","ended_at","type","location","game","stakes","buy_in","rebuys","cash_out","hands_played"},null,null,null,null,"started_at DESC","1")){
            return c.moveToFirst()?new Session(c.getLong(0),c.getLong(1),c.getLong(2),c.getString(3),c.getString(4),c.getString(5),c.getString(6),c.getDouble(7),c.getDouble(8),c.getDouble(9),c.getInt(10)):null;
        }
    }
