...
final class PokerStore extends SQLiteOpenHelper {
    static final class Session {
        ...
        String location, game, stakes;
        double buyIn, rebuys, cashOut;
        Session(long id,long startedAt,long endedAt,String location,String game,String stakes,double buyIn,double rebuys,double cashOut){
            this.id=id; this.startedAt=startedAt; this.endedAt=endedAt; this.location=location; this.game=game; this.stakes=stakes;
            this.buyIn=buyIn; this.rebuys=rebuys; this.cashOut=cashOut;
        }
        double result(){ return cashOut-buyIn-rebuys; }
        double hours(){ return Math.max(0,(endedAt-startedAt)/3600000.0); }
    }
    ...
    PokerStore(Context context){ super(context,"poker_manu.db",null,1); }

    @Override public void onCreate(SQLiteDatabase db){
        db.execSQL("CREATE TABLE sessions (id INTEGER PRIMARY KEY AUTOINCREMENT, started_at INTEGER NOT NULL, ended_at INTEGER NOT NULL, location TEXT NOT NULL, game TEXT NOT NULL, stakes TEXT NOT NULL, buy_in REAL NOT NULL, rebuys REAL NOT NULL, cash_out REAL NOT NULL)");
    ...
    long saveSession(long startedAt,long endedAt,String location,String game,String stakes,double buyIn,double rebuys,double cashOut){
        ContentValues v=new ContentValues();
        v.put("started_at",startedAt); v.put("ended_at",endedAt); v.put("location",clean(location,"Non renseigné"));
    ...
    List<Session> sessions(){
        ...
        try(Cursor c=getReadableDatabase().query("sessions",new String[]{"id","started_at","ended_at","location","game","stakes","buy_in","rebuys","cash_out"},null,null,null,null,"started_at DESC")){
            while(c.moveToNext()) out.add(new Session(c.getLong(0),c.getLong(1),c.getLong(2),c.getString(3),c.getString(4),c.getString(5),c.getDouble(6),c.getDouble(7),c.getDouble(8)));
        }
        return out;
    }

    Session lastSession(){
        try(Cursor c=getReadableDatabase().query("sessions",new String[]{"id","started_at","ended_at","location","game","stakes","buy_in","rebuys","cash_out"},null,null,null,null,"started_at DESC","1")){
            return c.moveToFirst()?new Session(c.getLong(0),c.getLong(1),c.getLong(2),c.getString(3),c.getString(4),c.getString(5),c.getDouble(6),c.getDouble(7),c.getDouble(8)):null;
        }
    }
